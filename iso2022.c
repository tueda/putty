...
int
iso2022_buflen (struct iso2022_data *thiss)
{
  return buflen (&thiss->rcv);
}

int
iso2022_tbuflen (struct iso2022_data *thiss)
{
  return buflen (&thiss->trns);
}

void
iso2022_tbufclear (struct iso2022_data *thiss)
{
  thiss->trns.buflen = thiss->trns.bufoff = 0;
}

void
iso2022_settranschar (struct iso2022_data *thiss, int value)
{
  thiss->trns.transchar = value;
}
...
unsigned char
iso2022_getbuf (struct iso2022_data *thiss)
{
  return getbuf (&thiss->rcv);
}

unsigned char
iso2022_tgetbuf (struct iso2022_data *thiss)
{
  return getbuf (&thiss->trns);
}

int
iso2022_width_sub (struct iso2022_data *thiss, wchar_t c)
{
  if (thiss->rcv.width == 0)
    {
      if (thiss->rcv.gr->type == UTF8CJK)
	return -1;
      else if (thiss->rcv.gr->type == UTF8NONCJK)
	return -2;
      return 1;			/* This shouldn't happen */
    }
  else
    return thiss->rcv.width;
}
...
q->buflen = wchar_to_utf8 (buf2, q->buf, 100, NULL); \
if (q->buflen)q->buflen--;}
/* Thiss is a workaround for wave dash problem */
...
void
iso2022_clearesc (struct iso2022_data *thiss)
{
  clearesc (&thiss->rcv);
}
...
static void
transmit (struct iso2022_data *thiss, struct iso2022struct *q, unsigned char c)
{
  if (q->buflen)
    q->buflen = q->bufoff = 0;
  q->buf[q->bufoff++] = c;
...
static void
transmit2 (struct iso2022_data *thiss, struct iso2022struct *q, unsigned char c)
{
  if (q->transchar) {
	transmit (thiss, q, c);
	return;
...
static void
autodetect_jp (struct iso2022_data *thiss, unsigned char *buf, int nchars)
{
  struct {
    int good;
    int bad;
...
#define DO_AUTODETECT(ENCODING) do { \
    if (thiss->autodetect.jp.ENCODING.e) \
      r = autodetect_jp_##ENCODING (&thiss->autodetect.jp.ENCODING, buf[i]); \
    else \
      r = AD_BAD; \
    if (r == AD_GOOD) \
      ENCODING.good++; \
...
#undef DO_AUTODETECT
  if (utf8cjk.good > 0 && utf8cjk.bad == 0)
    {
      if (thiss->rcv.lgr.type != UTF8CJK)
	{
	  thiss->rcv.lgr.type = UTF8CJK;
	  thiss->rcv.lgr.len = 0;
	  thiss->rcv.gr = &thiss->rcv.lgr;
	  thiss->rcv.buflen = thiss->rcv.bufoff = 0;
	}
    }
  else if (utf8cjk.bad > 0 && eucjp.good > 0 && eucjp.bad == 0)
    {
      if (thiss->rcv.lgr.type != JISX0208_1990)
	{
	  thiss->rcv.lgr.type = JISX0208_1990;
	  thiss->rcv.lgr.len = 2;
	  thiss->rcv.g2.type = JISX0201_KATAKANA;
	  thiss->rcv.g2.len = 1;
	  thiss->rcv.g3.type = JISX0212_1990;
	  thiss->rcv.g3.len = 2;
	  thiss->rcv.gr = &thiss->rcv.lgr;
	  thiss->rcv.buflen = thiss->rcv.bufoff = 0;
	}
    ...
    {
      if (thiss->rcv.lgr.type != MS_KANJI)
	{
	  thiss->rcv.lgr.type = MS_KANJI;
	  thiss->rcv.lgr.len = 2;
	  thiss->rcv.gr = &thiss->rcv.lgr;
	  thiss->rcv.buflen = thiss->rcv.bufoff = 0;
	}
...
void
iso2022_put (struct iso2022_data *thiss, unsigned char c)
{
  put (&thiss->rcv, c);
  if (thiss->rcv.switch_utf8 != SWITCH_UTF8_NONE)
    {
      if (thiss->rcv.switch_utf8 == SWITCH_UTF8_TO_UTF8)
	{
	  if (!thiss->rcv.usgr)
	    {
	      thiss->rcv.usgr = thiss->rcv.gr;
	      thiss->rcv.uslgr = thiss->rcv.lgr;
	      thiss->rcv.uslockgr = thiss->rcv.lockgr;
	      thiss->trns.usgr = thiss->trns.gr;
	      thiss->trns.uslgr = thiss->trns.lgr;
	      thiss->trns.uslockgr = thiss->trns.lockgr;
	      if (IS_CJK (thiss->rcv.usgr->type))
		thiss->rcv.lgr.type = UTF8CJK;
	      else
		thiss->rcv.lgr.type = UTF8NONCJK;
	      thiss->rcv.lgr.len = 0;
	      thiss->rcv.gr = &thiss->rcv.lgr;
	      thiss->rcv.lockgr = 1;
	      thiss->trns.lgr = thiss->rcv.lgr;
	      thiss->trns.gr = &thiss->trns.lgr;
	    }
	}
      else if (thiss->rcv.switch_utf8 == SWITCH_UTF8_FROM_UTF8)
	{
	  if (thiss->rcv.usgr)
	    {
	      thiss->rcv.gr = thiss->rcv.usgr;
	      thiss->rcv.lgr = thiss->rcv.uslgr;
	      thiss->rcv.lockgr = thiss->rcv.uslockgr;
	      thiss->trns.gr = thiss->trns.usgr;
	      thiss->trns.lgr = thiss->trns.uslgr;
	      thiss->trns.lockgr = thiss->trns.uslockgr;
	      thiss->rcv.usgr = 0;
	    }
	}
      thiss->rcv.switch_utf8 = SWITCH_UTF8_NONE;
    }
}

void
iso2022_autodetect_put (struct iso2022_data *thiss, unsigned char *buf,
			int nchars)
{
  if (thiss->autodetect.n)
    {
      if (thiss->autodetect.jp.n)
	autodetect_jp (thiss, buf, nchars);
    }
}

void
iso2022_transmit (struct iso2022_data *thiss, unsigned char c)
{
  transmit2 (thiss, &thiss->trns, c);
}
...
int
iso2022_init (struct iso2022_data *thiss, char *p, int mode)
{
  int i, f, j, k = 0;
  ...
  unsigned char *init_string = thiss->initstring;
  ...
  p += 8;
  {
	...
	if ((tmp_mskanji || tmp_big5 || tmp_utf8cjk || tmp_utf8noncjk ||
	    ...
	    && tmp_win95flag == thiss->win95flag
	    && tmp_lockgr == thiss->rcv.lockgr
	    && tmp_ssgr == thiss->rcv.ssgr
	    && tmp_mskanji == (thiss->rcv.lgr.type == MS_KANJI && thiss->rcv.gr == &thiss->rcv.lgr)
	    && tmp_big5 == (thiss->rcv.lgr.type == BIG5 && thiss->rcv.gr == &thiss->rcv.lgr)
	    && tmp_utf8cjk == (thiss->rcv.lgr.type == UTF8CJK && thiss->rcv.gr == &thiss->rcv.lgr)
	    && tmp_utf8noncjk == (thiss->rcv.lgr.type == UTF8NONCJK && thiss->rcv.gr == &thiss->rcv.lgr)
	    && tmp_autojp_eucjp == thiss->autodetect.jp.eucjp.e
	    && tmp_autojp_mskanji == thiss->autodetect.jp.mskanji.e
	    && tmp_autojp_utf8 == thiss->autodetect.jp.utf8cjk.e)
	  return 0;
      }
    memcpy (init_string, initstring, i);
  }
  iso2022_win95flag = thiss->win95flag = tmp_win95flag;
  init (&thiss->rcv);
  init (&thiss->trns);
  if (tmp_mskanji)
    {
  ...
  for (i = 0 ; thiss->initstring[i] ; i++)
    put (&thiss->rcv, thiss->initstring[i]);
  for (i++ ; thiss->initstring[i] ; i++)
    put (&thiss->trns, thiss->initstring[i]);
  if (tmp_lockgr)
    {
      thiss->rcv.lgr = *thiss->rcv.gr;
      thiss->rcv.gr = &thiss->rcv.lgr;
      thiss->rcv.lockgr = 1;
    }
  if (tmp_ssgr)
    thiss->trns.ssgr = 1;
  thiss->autodetect.n = 0;
  thiss->autodetect.jp.n = 0;
  thiss->autodetect.jp.eucjp.e = 0;
  thiss->autodetect.jp.mskanji.e = 0;
  thiss->autodetect.jp.utf8cjk.e = 0;
  if (tmp_autojp_eucjp)
    {
      thiss->autodetect.jp.eucjp.e = 1;
      thiss->autodetect.jp.eucjp.buflen = 0;
      thiss->autodetect.jp.n++;
      thiss->autodetect.n++;
    }
  if (tmp_autojp_mskanji)
    {
      thiss->autodetect.jp.mskanji.e = 1;
      thiss->autodetect.jp.mskanji.buflen = 0;
      thiss->autodetect.jp.n++;
      thiss->autodetect.n++;
    }
  if (tmp_autojp_utf8)
    {
      thiss->autodetect.jp.utf8cjk.e = 1;
      thiss->autodetect.jp.utf8cjk.buflen = 0;
      thiss->autodetect.jp.n++;
      thiss->autodetect.n++;
    }
#undef A
